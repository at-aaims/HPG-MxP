                   ...
                   const HPGMP_gen_opts& gopts)
{
    ...
    if (A.geom->rank == 0) { // Only PE 0 needs to compute and report timing results
        ...
        const bool is_benchmark = (gopts.run_type == run_t::benchmark || gopts.run_type == run_t::benchmark_no_ref);
        if (is_benchmark) {
    ...
    doc.get(DepartureFromSymmetry)->add("Departure for MG", testsymmetry_data.depsym_mg);*/
        ...
        const double refMGBytes    = c_ref.mg_rp.get_total_memory_bytes() + c_ref.mg_gs.get_total_memory_bytes();
        if (test_data.refTotalTime > 0.0) {
    ...
    doc.get("GB/s Summary")->add("Total with convergence and optimization phase overhead",
        ...
        if (test_data.optTotalTime > 0.0) {
            ...
            doc.get(membwstr)->add(" -   GS       (mxp)", c_opt.mg_gs.get_total_memory_bytes() / test_data.opt_times[8] / 1.0e9);
        ...
        if (test_data.refTotalTime > 0.0) {
            ...
            doc.get(membwstr)->add(" -   GS       (reference)", c_ref.mg_gs.get_total_memory_bytes() / test_data.ref_times[8] / 1.0e9);
 ...
 * specializations *
 * --------------- */
template void ReportResults< SparseMatrix<double>>(
    const SparseMatrix<double>&, int, const TestGMRESData&, int, const HPGMP_gen_opts&);

template void ReportResults< SparseMatrix<float>>(
