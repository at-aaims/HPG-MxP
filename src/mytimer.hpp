...
#define START_T() \
    fence();      \
    start_t = mytimer() //!< record current time in 'start_t'
#define STOP_T(t) \
    fence();      \
    t += mytimer() - start_t //!< store time difference in 't' using time in 'start_t'

// Stream-specific timers
//! record current time in _startvar after synchronizing the given stream.
#define TICK_STREAM_SYNC(_stream, _startvar) \
    fence(_stream);                          \
    _startvar = mytimer()
