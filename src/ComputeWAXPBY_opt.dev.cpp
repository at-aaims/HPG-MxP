...
__global__ void waxpby(const local_int_t n, const x_type alpha, const x_type* const __restrict__ xv,
                       const y_type beta, const y_type* const __restrict__ yv, w_type* const __restrict__ wv)
{
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        wv[i] = static_cast<w_type>(alpha) * static_cast<w_type>(xv[i]) + static_cast<w_type>(beta) * static_cast<w_type>(yv[i]);
    }
}
