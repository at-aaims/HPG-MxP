...
void Vector<scalar>::update_halos_pack_send_buffer(const DistMatrixBase* const mat) const
{
    ...
    if (MPI_SUCCESS != MPI_Waitall(num_neighbors, recv_reqs_.data(), MPI_STATUSES_IGNORE)) {
        printf(" Vector: update_halo: receives failed!");
        fflush(stdout);
        MPI_Abort(comm, -2025);
    }
    TOCK(time2);
...
__global__ void mxp_copy(const scalar_src* const __restrict__ src, scalar_dst* const __restrict__ dst, const int len)
{
