          ...
          const bool doPreconditioning, const bool verbose, TestGMRESData& test_data)
{
    ...
    while (niters <= max_iter && !converged)
    {
        ...
        TICK();
        ComputeWAXPBY(nrow, one, b, -one, Ap, r, A.isWaxpbyOptimized);
        TOCK(t11);
        ...
        flops_orth += (ifour*k*Nrow);
            ...
            START_T();
            ComputeGEMVT(nrow, k, one, P, Qk, zero, h, A.isGemvOptimized);
            STOP_T(t1);
            ...
            // beta = norm(Qk)
            START_T();
            ComputeDotProduct(nrow, Qk, Qk, beta, t4, A.isDotProductOptimized);
            STOP_T(t1);
        ...
        if (doPreconditioning) {
            ...
            ctrs.ortho.add_memory_traffic<scalar_type>((A.totalNumberOfRows + 1) * restart_length + A.totalNumberOfRows);
            ...
            // x += z
            TICK();
            ComputeWAXPBY(nrow, one, x, one, z, x, A.isWaxpbyOptimized);
            TOCK(t11);
            flops += (itwo * Nrow);
            ctrs.vecupd.add_memory_traffic<scalar_type>(3 * A.totalNumberOfRows);
        } else {
            // x += Q*t
            ComputeGEMV(nrow, k - 1, one, Q, t, one, x, A.isGemvOptimized);
            flops += (itwo * Nrow * (k - ione));
