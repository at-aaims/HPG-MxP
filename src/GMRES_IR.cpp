...
//#define SINGLEREDUCE_GMRES_IR
    ...
    while (niters <= max_iter && !converged)
    {
        ...
        TICK();
        ComputeWAXPBY(nrow, one_hi, b_hi, -one_hi, Ap_hi, r_hi, A.isWaxpbyOptimized);
        flops += (itwo * Nrow);
            ...
            if (use_mgs) {
                ...
                for (int j = 0; j < k; j++) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        // Qk = Qk - beta * Qj
                        START_T();
                        ComputeWAXPBY(nrow, one, Qk, -beta, Qj, Qk, A.isWaxpbyOptimized);
                        STOP_T(t2);
        ...
        if (doPreconditioning) {
            ...
            // x += z
            TICK();
            ComputeWAXPBY(nrow, one_hi, x_hi, one_hi, z_hi, x_hi, A.isWaxpbyOptimized);
            ctrs.vecupd.add_memory_traffic<scalar_type>(3 * A.totalNumberOfRows);
            flops += (itwo * Nrow);
            TOCK(t11);
#else
            ...
            // mixed-precision
            TICK();
            ComputeWAXPBY(nrow, one_hi, x_hi, one, z, x_hi, A.isWaxpbyOptimized);
            flops += (itwo * Nrow);
            TOCK(t11); // x += z
            ctrs.vecupd.add_memory_traffic<scalar_type>(2 * A.totalNumberOfRows);
