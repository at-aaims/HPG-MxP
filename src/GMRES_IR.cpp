...
//#define SINGLEREDUCE_GMRES_IR
    ...
    if (verbose && A.geom->rank == 0) {
        HPGMP_fout << std::endl
                   << " Running GMRES_IR(" << restart_length
    ...
    while (niters <= max_iter && !converged)
    {
        ...
        TICK();
        ComputeWAXPBY_opt(nrow, one_hi, b_hi, -one_hi, Ap_hi, r_hi, A.isWaxpbyOptimized);
        flops += (itwo * Nrow);
        TOCK(t11); // r = b - Ax (x stored in p)
        ...
        while (k <= restart_length && normr / normr0 > tolerance && !IS_NAN(normr))
        {
            ...
            if (use_mgs) {
                ...
                for (int j = 0; j < k; j++) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        // beta = Qk'*Qj
                        START_T();
                        ComputeDotProduct<Vector_type2, project_type>(nrow, Qk, Qj, beta, t4, A.isDotProductOptimized);
                        STOP_T(t1);
                        ctrs.ortho.add_memory_traffic<scalar_type2>(2 * A.totalNumberOfRows);

                        // Qk = Qk - beta * Qj
                        START_T();
                        ComputeWAXPBY_opt(nrow, one, Qk, -beta, Qj, Qk, A.isWaxpbyOptimized);
                        STOP_T(t2);
                ...
                START_T();
                ComputeGEMV(nrow, k, -one, P, h, one, Qk, A.isGemvOptimized);
                STOP_T(t2);
                t1_comp += h.time1;
                t1_comm += h.time2;
            ...
            START_T();
            Qk.scale(static_cast<scalar_type2>(one_pr / beta));
            STOP_T(t11);
            ctrs.vecupd.add_memory_traffic<scalar_type2>(2 * A.totalNumberOfRows);
            flops_orth += (Nrow);
            ...
            if (verbose && (k % print_freq == 0 || k + 1 == restart_length)) {
                ...
                if (verbose && A.geom->rank == 0) {
                    HPGMP_fout << "GMRES_IR Iteration = " << k << " (" << niters
                               << ")   Scaled Computed Residual = "
                               << normr << " / " << normr0 << " = " << normr / normr0;
#ifdef HPGMP_NUMERIC_CHECK
                    HPGMP_fout << " (True Residual = " << normr_hi << " / " << normr0_hi << " = "
                               << normr_hi / normr0_hi << ")";
        ...
        if (doPreconditioning) {
            ...
            // x += z
            TICK();
            ComputeWAXPBY_opt(nrow, one_hi, x_hi, one_hi, z_hi, x_hi, A.isWaxpbyOptimized);
            ctrs.vecupd.add_memory_traffic<scalar_type>(3 * A.totalNumberOfRows);
            flops += (itwo * Nrow);
            TOCK(t11);
#else
            ...
            // mixed-precision
            TICK();
            ComputeWAXPBY_opt(nrow, one_hi, x_hi, one, z, x_hi, A.isWaxpbyOptimized);
            flops += (itwo * Nrow);
            TOCK(t11); // x += z
            ctrs.vecupd.add_memory_traffic<scalar_type>(2 * A.totalNumberOfRows);
